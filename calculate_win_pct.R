...
CalculateTiebreakWinPct <- function(serveWinPct, returnWinPct){
  ...
  winTwoPoints <- serveWinPct * returnWinPct / 
                  (1 - serveWinPct * returnLosePct - serveLosePct * returnWinPct)
...
CalculateSetWinPct <- function(serveWinPct, returnWinPct){
  ...
  rn <- rep(NA, 10)
  for (i in 1:length(rn)){
    rn[i] <- 5 * (serveGameWinPct / serveGameLosePct)^i + 
             5 * (returnGameWinPct / returnGameLosePct)^i
  }
